...
public class ClaimService {
    ...
    public Page<ClaimResponse> findAll(String search, String vendor, String customer,
        ...
        Specification<Claim> dataScope = buildDataScopeSpec();
        Specification<Claim> combined = spec.and(dataScope);
        return claimRepository.findAll(combined, PageRequest.of(page, size)).map(this::toResponse);
    }
    ...
    public ClaimResponse create(ClaimRequest req) {
        if (claimRepository.existsByClaimNo(req.getClaimNo())) {
            throw new BusinessException("Duplicate claim no: " + req.getClaimNo());
        }
    ...
    public void delete(Long id) {
        ...
        if (claim.getStatus() != Claim.ClaimStatus.CANCELLED) {
            throw new BusinessException("Only cancelled claims can be deleted");
        }
    ...
    public ClaimResponse approveRCA(Long id, String comment, boolean isFinal) {
        ...
        if (isFinal) {
            ...
            claim.setRcaStatus(Claim.RcaStatus.PENDING_MANAGER);
    ...
    public ClaimResponse resetRCAToDraft(Long id) {
        Claim claim = claimRepository.findById(id)
                .orElseThrow(() -> new ResourceNotFoundException("Claim", id));
        if (claim.getRcaStatus() != Claim.RcaStatus.REJECTED) {
            throw new BusinessException("Only rejected RCA can be reset to draft");
        }
        claim.setRcaStatus(Claim.RcaStatus.DRAFT);
        addApprovalHistory(claim, "reset_to_draft", "Reset to draft for re-editing");
        auditLogService.log("RCA_RESET_DRAFT", "Reset RCA to draft for claim " + claim.getClaimNo());
        return toResponse(claimRepository.save(claim));
    }
    ...
    public List<Object[]> getVendorStats() {
        return applyDataScopeToStats(claimRepository.countByVendor());
    }

    public List<Object[]> getStatusStats() {
        return applyDataScopeToStats(claimRepository.countByStatus());
    }

    public List<Object[]> getDefectStats() {
        return applyDataScopeToStats(claimRepository.countByDefectCategory());
    }

    public List<Object[]> getInspectorStats() {
        return applyDataScopeToStats(claimRepository.countByInspector());
    }

    public List<Object[]> getRcaStatusStats() {
        return applyDataScopeToStats(claimRepository.countByRcaStatus());
    }

    public Map<String, Long> getRcaKpis() {
        User currentUser = getCurrentUser();
        String factoryAgent = (currentUser != null && currentUser.getRole() != User.Role.SUPERADMIN)
                ? currentUser.getFactoryAgent() : null;
        String inspector = (currentUser != null && currentUser.getRole() == User.Role.INSPECTOR)
                ? currentUser.getFullname() : null;

        LocalDate cutoff = LocalDate.now().minusDays(14);
        Map<String, Long> kpis = new java.util.LinkedHashMap<>();
        kpis.put("pending", countRcaPendingFiltered(factoryAgent, inspector));
        kpis.put("overdue", countRcaOverdueFiltered(cutoff, factoryAgent, inspector));
        kpis.put("approved", countRcaApprovedFiltered(factoryAgent, inspector));
        return kpis;
    }
    ...
    private List<Object[]> applyDataScopeToStats(List<Object[]> stats) {
        User currentUser = getCurrentUser();
        if (currentUser == null || currentUser.getRole() == User.Role.SUPERADMIN) {
            return stats;
        }
        String factoryAgent = currentUser.getFactoryAgent();
        String inspector = (currentUser.getRole() == User.Role.INSPECTOR)
                ? currentUser.getFullname() : null;

        Specification<Claim> dataScope = buildDataScopeSpec();
        List<Claim> filteredClaims = claimRepository.findAll(dataScope);

        java.util.Map<String, Long> vendorCounts = new java.util.LinkedHashMap<>();
        java.util.Map<Claim.ClaimStatus, Long> statusCounts = new java.util.LinkedHashMap<>();
        java.util.Map<String, Long> defectCounts = new java.util.LinkedHashMap<>();
        java.util.Map<String, Long> inspectorCounts = new java.util.LinkedHashMap<>();
        java.util.Map<Claim.RcaStatus, Long> rcaStatusCounts = new java.util.LinkedHashMap<>();

        for (Claim c : filteredClaims) {
            vendorCounts.merge(c.getVendor(), 1L, Long::sum);
            statusCounts.merge(c.getStatus(), 1L, Long::sum);
            if (c.getDefectCategory() != null) {
                defectCounts.merge(c.getDefectCategory(), 1L, Long::sum);
            }
            if (c.getInspector() != null) {
                inspectorCounts.merge(c.getInspector(), 1L, Long::sum);
            }
            if (c.getRcaStatus() != null) {
                rcaStatusCounts.merge(c.getRcaStatus(), 1L, Long::sum);
            }
        }

        List<Object[]> result = new java.util.ArrayList<>();
        for (var entry : vendorCounts.entrySet()) {
            result.add(new Object[]{entry.getKey(), entry.getValue()});
        }
        return result;
    }

    private long countRcaPendingFiltered(String factoryAgent, String inspector) {
        Specification<Claim> spec = buildDataScopeSpec();
        Specification<Claim> pendingSpec = (root, query, cb) ->
            root.get("rcaStatus").in(Claim.RcaStatus.SUBMITTED, Claim.RcaStatus.PENDING_MANAGER);
        return claimRepository.count(spec.and(pendingSpec));
    }

    private long countRcaOverdueFiltered(LocalDate cutoff, String factoryAgent, String inspector) {
        Specification<Claim> spec = buildDataScopeSpec();
        Specification<Claim> overdueSpec = (root, query, cb) -> cb.and(
            root.get("rcaStatus").in(Claim.RcaStatus.SUBMITTED, Claim.RcaStatus.PENDING_MANAGER, Claim.RcaStatus.DRAFT),
            cb.isNotNull(root.get("qcInformDate")),
            cb.lessThanOrEqualTo(root.get("qcInformDate"), cutoff)
        );
        return claimRepository.count(spec.and(overdueSpec));
    }

    private long countRcaApprovedFiltered(String factoryAgent, String inspector) {
        Specification<Claim> spec = buildDataScopeSpec();
        Specification<Claim> approvedSpec = (root, query, cb) ->
            cb.equal(root.get("rcaStatus"), Claim.RcaStatus.APPROVED);
        return claimRepository.count(spec.and(approvedSpec));
    }
    ...
    private Specification<Claim> buildDataScopeSpec() {
        return (root, query, cb) -> {
            User currentUser = getCurrentUser();
            if (currentUser == null) {
            ...
            if (factoryAgent != null && !factoryAgent.isBlank()) {
                predicates.add(cb.equal(root.get("factoryAgent"), factoryAgent));
            }

            if (role == User.Role.INSPECTOR) {
                predicates.add(cb.equal(root.get("inspector"), currentUser.getFullname()));
            }
    ...
    public Claim findByIdEntity(Long id) {
