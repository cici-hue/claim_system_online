...
public interface ClaimRepository extends JpaRepository<Claim, Long>, JpaSpecificationExecutor<Claim> {
    ...
    @Query("SELECT c FROM Claim c WHERE c.rcaStatus IN ('SUBMITTED', 'PENDING_MANAGER', 'DRAFT') " +
    ...
    @Query("SELECT COUNT(c) FROM Claim c WHERE c.rcaStatus IN ('SUBMITTED', 'PENDING_MANAGER')")
    long countRcaPending();

    @Query("SELECT COUNT(c) FROM Claim c WHERE c.rcaStatus = 'APPROVED'")
    long countRcaApproved();

    @Query("SELECT COUNT(c) FROM Claim c WHERE c.rcaStatus IN ('SUBMITTED', 'PENDING_MANAGER', 'DRAFT') " +
           "AND c.qcInformDate IS NOT NULL AND c.qcInformDate <= :cutoff")
    ...
    @Query("SELECT YEAR(c.claimDate), MONTH(c.claimDate), COUNT(c) " +
           "FROM Claim c WHERE c.claimDate >= :since GROUP BY YEAR(c.claimDate), MONTH(c.claimDate) " +
           "ORDER BY YEAR(c.claimDate) ASC, MONTH(c.claimDate) ASC")
    List<Object[]> countByMonth(@Param("since") LocalDate since);

    @Query("SELECT COUNT(c) FROM Claim c WHERE c.factoryAgent = :factoryAgent AND c.createdAt >= :since")
    long countNewClaimsByFactoryAgent(@Param("factoryAgent") String factoryAgent, @Param("since") LocalDateTime since);
}
