...
const UsersPage: React.FC = () => {
  ...
  const openAdd = () => {
    if (!canManageUsersPage()) {
      showToast('Super Admin or Admin permission required')
      return
    }
    setNewUserEmail('')
    setNewUserPassword('')
  ...
  const handleSaveNew = async (e: React.FormEvent) => {
    ...
    if (!newUserPassword) {
      showToast('Please set an initial password.')
      return
    }
    if (!newUserFactoryAgent) {
      showToast('Please select a factory agent.')
      return
    }
    ...
    if (users.find(u => u.username === username)) {
      showToast(`Username "${username}" already exists. Please use a different email.`)
      return
    }
  ...
  const handleDelete = async (id: number, username: string) => {
    ...
    if (users.length <= 1) {
      showToast('Cannot delete the last admin user')
      return
    }
  ...
  return (
    <div>
      ...
      <div style={{ background: '#fff', borderRadius: 12, padding: '20px', marginBottom: 20,
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13, minWidth: isMobile ? 700 : 'auto' }}>
          <thead>
